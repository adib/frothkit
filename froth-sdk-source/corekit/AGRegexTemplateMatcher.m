...
@implementation AGRegexTemplateMatcher


+ (AGRegexTemplateMatcher *)matcherWithTemplateEngine:(MGTemplateEngine *)theEngine
{
	return [[[AGRegexTemplateMatcher alloc] initWithTemplateEngine:theEngine] autorelease];
}
...
- (NSDictionary *)firstMarkerWithinRange:(NSRange)range
{
	...
	if (matchRange.length > 0) {
		...
		markerRange = [matchString rangeOfRegex:regex inRange:localRange capture:2 + offset];
		...
		if (markerRange.length > 0) {
			...
			if (filterRange.length > 0) {
				...
				NSString *space = @" ";
				NSRange filterArgDelimRange = [filterString rangeOfRegex:@":(?:\\s+)?" inRange:localRange 
																 capture:0];
...
- (NSArray *)argumentsFromString:(NSString *)argString
{
	...
	while (location != NSNotFound) {
		...
		NSRange entireRange = [argString rangeOfRegex:argsPattern inRange:searchRange capture:0];
		NSRange matchedRange = [argString rangeOfRegex:argsPattern inRange:searchRange capture:1];
		if (matchedRange.length == 0) {
			matchedRange = [argString rangeOfRegex:argsPattern inRange:searchRange capture:2];
			if (matchedRange.length == 0) {
				matchedRange = [argString rangeOfRegex:argsPattern inRange:searchRange capture:3];
			}
		}
